...
-(NSURLSessionUploadTask *)QiniuOperation:(NSInteger)index sourceData:(NSData*)sourceData{
  ...
  NSURLSessionUploadTask *task =   [[RequestClient sharedClient]uploadTaskWithUrlString:kQiniuUpUploadUrl parameters:parameters mimeType:theFile.mimeType sourceData:sourceData progress:^(NSProgress * _Nonnull downloadProgress) {
         
         
         if (self.uploadOneFileProgress) {
             self.uploadOneFileProgress(nil, index, downloadProgress.completedUnitCount);
         }
         ...
         if(!error){
             if (self.uploadOneFileSucceeded) {
                 self.uploadOneFileSucceeded(response,index,responseObject[@"key"]);
             ...
             if (self.uploadOneFileFailed) {
                 self.uploadOneFileFailed(response, index, [error copy]);
             }
         }
...
- (void)getQiniuFileSourceDataAndCreateOperation:(NSInteger)index
{
    ...
    if (qiniuFile.rawData) {
        ...
        NSBlockOperation *theOperation = [NSBlockOperation blockOperationWithBlock:^{
            [self QiniuOperation:index sourceData:qiniuFile.rawData];
        }];
