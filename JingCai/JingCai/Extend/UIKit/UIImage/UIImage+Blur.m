...
#define ENABLE_TINT                     1
    ...
    if (!self.CGImage)
    {
        DLog(@"*** error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage)
    {
        DLog(@"*** error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
    ...
    if (hasBlur || hasSaturationChange)
    {
        ...
        if (e != kvImageNoError)
        {
            DLog(@"*** error: vImageBuffer_InitWithCGImage returned error code %zi for inputImage: %@", e, self);
