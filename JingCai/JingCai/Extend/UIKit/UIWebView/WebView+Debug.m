...
void enableRemoteWebInspector() {
    ...
    DLog(@"Point your browser at http://%@:9999", getAddress());
}
...
- (void)webView:(WebView *)webView      didParseSource:(NSString *)source
 baseLineNumber:(unsigned)lineNumber
    ...
    if (kDidParseSource)
        DLog(@"ScriptDebugger called didParseSource: \nsourceId=%d, \nurl=%@", sid, url);
}
...
- (void)webView:(WebView *)webView failedToParseSource:(NSString *)source
 baseLineNumber:(unsigned)lineNumber
    ...
    if (kFailedToParseSource)
        DLog(@"ScriptDebugger called failedToParseSource:\
              \nurl=%@ \nline=%d \nerror=%@ \nsource=%@",
...
- (void)webView:(WebView *)webView    didEnterCallFrame:(WebScriptCallFrame *)frame
    ...
    if (kDidEnterCallFrame)
        DLog(@"ScriptDebugger didEnterCallFrame:\
              \nsourceId=%d \nline=%d \nfunction=%@, \ncaller=%@, \nexception=%@", 
              sid, 
              lineno, 
              [self functionNameForFrame:frame], 
...
- (void)webView:(WebView *)webView willExecuteStatement:(WebScriptCallFrame *)frame
    ...
    if (kWillExecuteStatement)
        DLog(@"ScriptDebugger willExecuteStatement:\
              \nsourceId=%d \nline=%d \nfunction=%@, \ncaller=%@, \nexception=%@", 
              sid, 
              lineno, 
              [self functionNameForFrame:frame], 
...
- (void)webView:(WebView *)webView   willLeaveCallFrame:(WebScriptCallFrame *)frame
    ...
    if (kWillLeaveCallFrame)
        DLog(@"ScriptDebugger willLeaveCallFrame:\
              \nsourceId=%d \nline=%d \nfunction=%@, \ncaller=%@, \nexception=%@", 
              sid, 
              lineno, 
              [self functionNameForFrame:frame], 
